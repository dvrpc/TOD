...
var stations = L.geoJson(null, {
  ...
  style: function (feature) {
    switch (feature.properties.QUAD) {
      case "aboveabove":
        return { fillColor: "#124A62", radius: 10 };
      case "abovebelow":
        return { fillColor: "#1C7AA0", radius: 7 };
      case "belowabove":
        return { fillColor: "#1C7AA0", radius: 7 };
      case "belowbelow":
        return { fillColor: "#28AAE1", radius: 5 };
    }
  },
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      layer.bindLabel(
        "<u>" +
          feature.properties.Station_Na +
          "</u><br>Existing TOD Orientation:&nbsp" +
          numeral(feature.properties.ExistingOr).format("0.00") +
          "<br>Future TOD Potential:&nbsp" +
          numeral(feature.properties.FuturePote).format("0.00"),
        {
...
var CartoDB_Positron = L.tileLayer(
  "https://cartodb-basemaps-{s}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png?key=" +
    env.CARTO_API_KEY,
  {
    maxZoom: 18,
    attribution:
...
var Mapbox_Imagery = L.tileLayer(
  "https://api.mapbox.com/styles/v1/crvanpollard/cimpi6q3l00geahm71yhzxjek/tiles/{z}/{x}/{y}?access_token=pk.eyJ1IjoiY3J2YW5wb2xsYXJkIiwiYSI6Ii00ZklVS28ifQ.Ht4KwAM3ZUjo1dT2Erskgg",
  {
    ...
    zoomOffset: -1,
    attribution:
      '© <a href="https://www.mapbox.com/map-feedback/">Mapbox</a> © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',
  },
);
...
L.Control.MapLegend = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    legend_body.innerHTML +=
      '<div id="legend-content" class="row">' +
    ...
    legend_top.innerHTML +=
      '<p><b>Rating TOD Opportunities</b><span id="legend-definition" class="nav-item" data-modal="#rateModal" onclick="legendraw5()"><i class="glyphicon glyphicon-info-sign"></i>&nbsp;&nbsp;data definitions</span></p>';
...
function identify(e) {
  ...
  if (props.OPERATOR === "PATCO") {
    var Information = "<div>" + props.MAP_NAME + " - " + props.LINE + "</div>";
  } else {
    var Information =
      "<div>" +
      props.MAP_NAME +
  ...
  var stationinfo =
    "<div style='margin-left: 25px;'> Location: <b>" +
    props.LOC +
    ", " +
    props.CNTY +
    " County</b></div>" +
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw3()'></i> Station Area Type: <b>" +
    props.Type_1 +
    "</b></div>" +
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw4()'></i> Planning Area Context: <b>" +
    props.LUContext +
    "</b></div>";

  var existingOVinfo =
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw()'></i><b>Existing TOD Orientation:</b>&nbsp;&nbsp; <h2 style='display: inline'>" +
    numeral(props.ExistingOr).format("0.00") +
    " / 4</h2></div>";
  ...
  if (words === "Philly:") {
    var TRAVEL =
      "<div><b>Travel Time to NYC:</b>&nbsp;" +
      props.Time_Data.split(" ")[3] +
      "<i> transit to auto travel time ratio</i></div>";
  } else {
    var TRAVEL =
      "<div><b>Travel Time to Philly:</b>&nbsp;" +
      numeral(props.Time_Data).format("0.00") +
      " <i> transit to auto travel time ratio</i></div>";
  }

  var existinginfo =
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw()'></i><b <b style='font-size: 14px;'>Existing TOD Orientation Category</b></div>" +
    "<br><div><b>Transit Service Quality:</b>&nbsp; " +
    props.TCI_Data +
    " <i>TCI Score</i></div>" +
    "<div><b>Job Access:</b>&nbsp;" +
    numeral(props.Job_Data).format("0,0") +
    " <i> jobs accessible within 30 minute transit ride</i></div>" +
    //  +"<div><b>Travel Time:</b>&nbsp;" + numeral(props.Time_Data).format('0.00') +" <i>transit to auto travel time ratio</i></div>"
    TRAVEL +
    "<div><b>Intensity:</b>&nbsp; " +
    numeral(props.Int_Data).format("0,0") +
    " <i> residents and workers within a half-mile of the station</i></div>" +
    "<div><b>Car Ownership:</b>&nbsp; " +
    props.Car_Data +
    "% <i>housing units with zero or one vehicle</i></div>" +
    "<div><b>Non-Car Commuters:</b>&nbsp; " +
    props.Com_Data +
    "%<i> residents who commute to work by public transportation, bicycle, or walking</i></div>" +
    "<div><b>Walk Score®:</b>&nbsp; " +
    props.Walk_data +
    "</div>";

  var futureOVinfo =
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw2()'></i><b>Future TOD Potential: </b> &nbsp;&nbsp; <h2 style='display: inline'>" +
    props.FuturePote +
    " / 4</h2></div>";

  var futureinfo =
    "<div><i class='glyphicon glyphicon-info-sign' id='info-icon' onclick='legendraw2()'></i><b style='font-size: 14px;'>Future TOD Potential Categories</b></div>" +
    "<br><div><b>Development Activity:</b>&nbsp; " +
    props.Dev_Data +
    " <i>multifamily units recently built, under construction, or proposed</i></div>" +
    "<div><b>Commercial Market:</b>&nbsp;$" +
    props.CommRent_D +
    " <i>5-year average rent per square foot for office space</i></div>" +
    "<div><b>Residential Market:</b>&nbsp;$" +
    props.ResRent_Da +
    " <i>average asking rent per square foot for multifamily units</i></div>" +
...
function FTODPdraw(value) {
  $("#FTODPModal")
    .one("shown.bs.modal", function () {
      $('#FTODPTabs a[data-target="#' + value + '"]').tab("show");
    })
    .modal("show");
}
...
function EXTODdraw(value) {
  $("#EXTODModal")
    .one("shown.bs.modal", function () {
      $('#EXTODTabs a[data-target="#' + value + '"]').tab("show");
    })
    .modal("show");
 ...
 */ tooltip: {
  ...
  $(".highcharts-xaxis-labels text, .highcharts-xaxis-labels span").click(
    function () {
